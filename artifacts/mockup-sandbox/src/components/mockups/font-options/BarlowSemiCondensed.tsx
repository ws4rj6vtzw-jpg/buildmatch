export function BarlowSemiCondensed() {
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Barlow+Semi+Condensed:wght@400;500;600;700;800;900&display=swap');
        .font-preview * { font-family: 'Barlow Semi Condensed', sans-serif !important; }
      `}</style>
      <AppPreview fontName="Barlow Semi Condensed" />
    </>
  );
}
  ...
  return (
    <div className="font-preview" style={{ background: "#F0F5FF", minHeight: "100vh", display: "flex", flexDirection: "column", width: 320 }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #DBEAFE", padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: 20, fontWeight: 900, letterSpacing: "-0.5px", color: "#0A1628", textTransform: "uppercase" }}>BUILDMATCH</span>
      ...
      <div style={{ margin: "0 14px", background: "#fff", borderRadius: 18, border: "1px solid #DBEAFE", overflow: "hidden", flex: 1, maxHeight: 420, display: "flex", flexDirection: "column" }}>
        {/* Photo area */}
        <div style={{ background: "linear-gradient(135deg, #1E3152 0%, #3B82F6 100%)", height: 220, position: "relative", display: "flex", alignItems: "flex-end", padding: 16 }}>
          <div style={{ position: "absolute", top: 14, right: 14, background: "rgba(0,0,0,0.4)", borderRadius: 8, padding: "4px 8px", display: "flex", alignItems: "center", gap: 4 }}>
        ...
        <div style={{ padding: "14px 16px", flex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            {[1,2,3,4,5].map(s => <span key={s} style={{ color: s <= 5 ? "#F59E0B" : "#DBEAFE", fontSize: 14 }}>★</span>)}
